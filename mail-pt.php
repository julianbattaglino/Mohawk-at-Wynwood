...
if (!$response->isSuccess()) {
    echo "<script> alert('Erro: o token reCAPTCHA v3 é inválido. Tente novamente.');
    ...
    if ($score < 0.7) {
        echo "<script> alert('Erro: a pontuação do reCAPTCHA v3 é muito baixa, tente novamente.');
             window.location.href = 'index-pt.html';
             </script>";
    } else {
        ...
        try {
            ...
            $mail->Subject = 'Mohawk Formulario completado';
            $mail->Body = $body;
            ...
            echo "
    <script> alert('Obrigado por nos contatar. Nós responderemos o mais rapidamente possível.');
    window.location.href = 'index-pt.html';
    </script>";
        } catch (Exception $e) {
            echo "
    <script> alert('Ocorreu um erro, não foi possível enviar a mensagem, tente novamente.');
    window.location.href = 'index-pt.html';
    </script>";
        }
    }
